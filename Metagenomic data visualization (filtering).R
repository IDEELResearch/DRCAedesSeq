...
### Relative abundance analysis at the family level 
...
### Relative abundance analysis at the genus level 
genus_RA <- genus_RPM
...
### PCA analysis based on the viral genus annotation
PCA_df <- read_excel("PCA_analysis_genus_filtering.xlsx")
...
blast_result <- read_excel("BLAST_KrakenUniq_results_filtering.xlsx")
...
## The file "all_ref.xlsx" contains information on all published DENV sequences from the NCBI Virus database, downloaded on June 25, 2025. 
...
## Select the longest reference sequences representing different countries and collection years
result <- all_ref %>%
...
## Select references to ensure representation across diverse geographical regions and serotypes.
...
## Final list of reference sequences included in the Pan-DENV phylogenetic analysis conducted in this study. Some references were excluded due to high genetic diversity compared to others of the same serotype. A Zika virus sequence was used as the outgroup.
...
## Download references from the NCBI database
...
## Select the longest sequence representing each country and collection year
DENV2_longest <- DENV2_ref %>%
     group_by(Collection_year, Country) %>%
     slice_max(order_by = Length, n = 1, with_ties = FALSE) %>%
     ungroup()

## Select reference sequences representing diverse geographic regions
DENV2_random_ref <- DENV2_longest %>%
  group_by(continent) %>%
  slice_sample(n=15) %>%
  ungroup()

## Final list of references included in the DENV-2 phylogenetic analysis. Some references with genotype information from a published study (PMID: 34188091) were included. A DENV-4 sequence was used as the outgroup.
...
## Final list of references included in the DENV-2 phylogenetic analysis. Some references with genotype information from a published study (PMID: 37293986) were included. A DENV-2 sequence was used as the outgroup.
...
## Load the phylogenetic tree for visualization. Three separate trees were generated for human blood-associated dicistrovirus because reads mapped to different regions of the genome.
