...
for i in *.fastq; do krakenuniq --db viral/DBDIR/ --classified-out align_krakenuniq_viral/classified_sequences/${i%.fastq}classfied_sequences.fastq --threads 16 --report-file align_krakenuniq_viral/${i%.fastq}_not_mpa.report  --output align_krakenuniq_viral/${i%.fastq}_not_mpa.txt ${i} ; done 
...
for i in *.fastq; do krakenuniq --db nt/DBDIR/ --classified-out align_krakenuniq_nt/classified_sequences/${i%.fastq}classfied_sequences.fastq --threads 16 --report-file align_krakenuniq_nt/${i%.fastq}_not_mpa.report  --output align_krakenuniq_viral/${i%.fastq}_not_mpa.txt ${i} ; done 
